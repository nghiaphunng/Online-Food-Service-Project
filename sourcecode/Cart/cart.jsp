...
<body>
		...
		<ul>
			<li>
				<a href="#" class="active">
					...
					<span class="links_name">Chờ xác nhận</span>
				</a>
			</li>
			...
			<li>
				<a href="#">
					<i class='bx bx-user'></i>
					<span class="links_name">Chờ giao hàng</span>
				</a>
			</li>
	...
	<section class="home_section">
		...
		<div class="choxacnhan" id="choxacnhan">
			<c:forEach items="${cartItemStatus0 }" var="cart">
					...
					<thead>
						...
						<c:forEach items="${cart.value }" var="item">
							<tr>
							    ...
							    <td><input type="number" value="${item.getQuantityItem() }" min="0" class="quantity" onchange="updateQuantityItemCart(${item.getCartId()}, ${item.getItemId()}, this.value)"></td>
				...
				<div class="restaurant-total">
				    <button class="pay-button" onclick="updateCartStatus(${cart.value[0].getCartId()}, 1)">Xác nhận mua hàng</button>
				    <span>Tổng tiền: </span>
	...
	<div class="chogiaohang">
		<c:forEach items="${cartItemStatus2 }" var="cart">
			<div class="main-choxacnhan" id="main-choxacnhan">
				<h3>${cart.value[0].getNameRestaurant() }</h3>
				<table>
					<thead>
			    ...
			    <div class="restaurant-total">
			        <button class="pay-button" onclick="updateCartStatus(${cart.value[0].getCartId()}, 3)">Đã nhận hàng</button>
			        <span>Tổng tiền: </span>
			        <span class="total-price">0d</span>
				</div>
			</div>
...
<script>
    document.querySelector(".choxacnhan").style.display = "block";
    document.querySelector(".chonhahang").style.display = "none";
    document.querySelector(".chogiaohang").style.display = "none";
    ...
    links.forEach(link => {
      link.addEventListener("click", function(event) {
        ...
        if (linkText === "Chờ xác nhận") {
          document.querySelector(".choxacnhan").style.display = "block";
          document.querySelector(".chonhahang").style.display = "none";
          document.querySelector(".chogiaohang").style.display = "none";
          document.querySelector(".thanhcong").style.display = "none";
          
        } else if (linkText === "Chờ nhà hàng xác nhận") {
          document.querySelector(".choxacnhan").style.display = "none";
          document.querySelector(".chonhahang").style.display = "block";
          document.querySelector(".chogiaohang").style.display = "none";
          document.querySelector(".thanhcong").style.display = "none";
    
        } else if (linkText === "Chờ giao hàng") {
          document.querySelector(".choxacnhan").style.display = "none";
          document.querySelector(".chonhahang").style.display = "none";
          document.querySelector(".chogiaohang").style.display = "block";
          document.querySelector(".thanhcong").style.display = "none";
